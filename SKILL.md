---
name: 国际站速卖通中转英
description: 把中文电商详情页图片里的文案意译成有品牌高级感的英文，并生成「中文原图 + 中英对照表」的 Word 文档。对照表一行一个句对（左列中文原文、右列英文文案），逐句对齐、便于逐行核对；单元格行距加大、句与句之间留白充足，长表也易读；尺寸标注板块一个尺寸只占一行、并附中文提示说明「部位文案仅为定位作用」（多个尺寸挤在一行则不直观）；图内没有可翻译文案的行，英文栏统一写「/」避免被误当成上架文案；文案密集的长图按图内自然空白带自动切分，每段图下紧跟该段文案；图内重复出现的角标不逐图重复翻译、但保留一处放在文首醒目说明；固定文案与术语走统一口径（碳素钢→Metal、灯杆→Lamp Pole、底座→Base、灯罩→Lampshade，并有客户指定固定句逐字照抄），颜色名统一走品牌色卡口径。适用于阿里巴巴国际站（Alibaba.com）、速卖通（AliExpress）、亚马逊等跨境平台上新时，把国产产品的中文详情页本地化为英文详情页。触发词：国际站速卖通中转英、详情页翻译、中文详情页转英文、Listing 本地化、中英对照 Word、逐句对照、句级对照、表格对照、行距调整、无文案斜杠、尺寸分行、尺寸板块、尺寸标注图、部位文案提示、颜色口径统一、固定文案表、统一名词翻译、术语统一、产品文案出海、详情页图片英文文案、详情页翻英文、长图切分、长图自动切割、图片切割比对。
author: Howie
agent_created: true
---
...
3. **角标不重复翻译，但至少保留一处、且必须在文首醒目位置** —— 同一枚角标出现在多张图时，**只在文首「注」区块里写一次完整的（中文原文 + 统一英文写法 + 出现图号）**，并让它**单独成段、加粗或居中**，一眼能看到；正文各图**不再重复列出**。
   ❌ 每张图都抄一遍角标（灌水）❌ 全文一处都不留（读者不知道角标存在）—— 两种都不许。
...
8. **固定文案表逐字照抄** —— `references/translation-playbook.md` **第〇节固定文案表**是客户指定口径（含 `金属为骨，光影为魂`、`碳素钢`、`灯杆`、`底座`、`灯罩`、`一盏灯，照见三代人的团圆` 六条），**优先级高于一切通用术语**：中文原文只要出现，英文必须照抄表内写法，**不许换词、缩写、调序或改标点**。生成前过一遍闸门：`python scripts/glossary_check.py <作业.json>`（退出码 2 = 有强制项不一致，先改文案再生成）。
9. **尺寸板块：一个尺寸一行 + 必须带中文提示** —— 尺寸标注图里引线多、数值多，**一个尺寸 = 一个句对（一行）**，❌ 不许把 `225mm / 485mm / 260mm / 25mm / ø120mm` 挤进同一行（挤在一起分不清哪条引线对哪个数，客户原话「放在一起不直观」）；**部位名称单独成行**、中文列写「部位文案（仅定位）：」，**英文列要连贯翻译** —— 原图那串中文标签本身就是文案，须译成自然的英文名词短语（`碳素钢底座` → `Metal Base`、`白玉磨砂玻璃` → `Frosted White Jade Glass`），❌ **不许用破折号 / 斜杠把部位与材质拼起来**（`Base — Metal` 生硬，客户明确反对）；语序按英文习惯「材质 + 部位」，拼写仍照第〇节固定文案表。该板块**必须带一条中文提示**（客户指定口径，逐字）：
   > 部位文案仅为定位作用，如原图只有尺寸则忽略该英文文案，只取用 mm 及 inch 尺寸。

   作业里给该块写 `"kind": "size"`：脚本会自动带上这句提示（要改措辞就显式写 `"tip_cn"`），并**自动把「一行多个尺寸」拆成一行一个尺寸**；`verify_docx.py` 复核「含 ≥2 个尺寸的行 = 0」与提示段数量。详见手册 4.1。
...
- **第一步先过「第〇节固定文案表」**：`金属为骨，光影为魂` / `碳素钢` / `灯杆` / `底座` / `灯罩` / `一盏灯，照见三代人的团圆`
  —— 命中就**逐字照抄表内英文**（含标点大小写），**不要另译**；这一层优先于所有通用术语与意译风格。
- 意境类（设计理念 / slogan）走**意译 + 品牌调性**（但若命中固定文案表，则照抄，不意译）；
...
- **尺寸板块一个尺寸一行**：`225mm` / `485mm` / `260mm` / `25mm` / `Ø120mm` **各占一行**，
  **不要**挤进一个句对；部位名称（底座 / 灯杆 / 灯罩…）单独成行，中文列写「部位文案（仅定位）：」。
  该块写 `"kind": "size"`，脚本会自动带中文提示、并把多尺寸行自动拆开（手册 4.1）。
...
- **固定文案表命中即照抄**：中文是「金属为骨，光影为魂 / 碳素钢 / 灯杆 / 底座 / 灯罩 / 一盏灯，照见三代人的团圆」时，
  英文用第〇节表内写法（`Metal as the framework, light as the soul.` / `Metal` / `Lamp Pole` / `Base` / `Lampshade` /
  `A single lamp, illuminates the reunion of three generations.`），**逐字、含句末句号**。
  不要写 Carbon Steel、Lamp Post、Base Plate、Backplate、Shade、The lamp lights up three generations' reunion 等变体。
- 断句完成后、生成前，跑一次口径闸门：`python scripts/glossary_check.py <作业.json>` —— **退出码 2 就先改文案别硬生成**。
...
作业文件用 `pairs` 存句对，`brand` / `target_lang` 存开工前确认的结果；**尺寸标注块另加 `kind` / `tip_cn` 两个字段**：
```json
{
  "title_cn": "...", "title_en": "...",
  "brand": "Aisilan",
  ...
  "blocks": [
    { "img": "slices/BD111_06_b.jpg",
      ...
      "pairs": [ ["MATERIAL　材质：玻璃、碳素钢", "MATERIAL: Glass, Metal"], ["…", "…"] ] },
    { "img": "slices/BD111_06_a.jpg",
      "cap": "图 06 · 尺寸标注（图上段 1/2） / Dimensions (Part 1/2)",
      "kind": "size",
      "tip_cn": "部位文案仅为定位作用，如原图只有尺寸则忽略该英文文案，只取用 mm 及 inch 尺寸。",
      "pairs": [ ["部位文案（仅定位）：碳素钢底座", "Metal Base"],
                 ["部位文案（仅定位）：白玉磨砂玻璃", "Frosted White Jade Glass"],
                 ["尺寸标注：225mm", "225mm (8.86in)"],
                 ["尺寸标注：485mm", "485mm (19.09in)"] ] }
  ]
}
```
| 字段 | 作用 |
|---|---|
| `kind: "size"` | 标记本块为**尺寸板块**：脚本自动带上固定中文提示，并把「一行多个尺寸」的行**自动拆成一行一个尺寸** |
| `tip_cn` | 该块的中文提示（字符串，或数组 = 多行；**传 `""` = 明确不要提示**）。不写而 `kind` 是尺寸类 → 用固定那句 |
| `no_split: true` | 极少数情况下不要自动拆行（默认不写 = 允许自动拆） |
...
每块插入的 HTML 结构（尺寸板块多一行琥珀色中文提示，排在图注之后、表格之前）：
```html
<p style="font-size:1pt">&nbsp;</p>
<p style="text-align:center;font-size:9pt;color:#999999">图 06 · …（图下段 2/2） / … (Part 2/2)</p>
<p style="font-size:9pt;color:#B26A00"><b>※ 提示：</b>部位文案仅为定位作用，如原图只有尺寸则忽略该英文文案，只取用 mm 及 inch 尺寸。</p>
...
- **尺寸板块的中文提示**（`※ 提示：…`）排在**图注之后、表格之前**，用琥珀色 `#B26A00` 与「灰中文 / 蓝英文」区分开 —— 它是给运营看的判读口径，**不是上架文案**；块里写 `"kind": "size"` 自动生成，换措辞用 `"tip_cn"`。

### Step 6 验证（不要依赖渲染服务）
```bash
PYTHONIOENCODING=utf-8 <python> scripts/verify_docx.py <输出.docx> <期望块数> <期望句对数>
```
自检项：图片数 = 图注数 = 块数 = **表格数**；表格行数 = 句对数 + 表头行数；**英文格无空格**（无译文行必须是 `/`）；`tcBorders = (行数 − 表格数) × 2`；图注段均 `jc=center`；`<w:keepNext` ≈ 块数 × 2；**`w:spacing w:line` ≈ (句对 + 表头) × 2**（行距生效）；**`tblCellMar` 的 top/bottom ≥ 60dxa**（句距生效）；**`size rows = 0`**（没有一行塞两个尺寸）、**`tips` ≥ 尺寸块数**（尺寸板块的中文提示在位）。
...
**总口径（客户明确要求）：角标不重复翻译，但至少保留一处 —— 那一处放在文首、要醒目。**

1. 先在文首「注」里写**完整的一次**：**中文文案 + 统一英文写法 + 说明它出现在哪些图**，并让它**单独成段、加粗或居中**，一眼看得到。
   > **角标说明（统一口径，正文不再重复）**：图 02–05、08–13 图内右下角均为同一枚产品角标，
   > 中文原文「… · DESIGN BY SAKA · 金属为骨，光影为魂」，
   > 英文统一写法「… · Designed by Aisilan · Metal as the framework, light as the soul.」。
2. 正文各块**删掉角标行**；若某图**只有**角标没有其他文案，用一个句对占位（中文格说明 + 英文格 `/`），别留空块。
...
5. 角标里的 slogan 段走**手册第〇节固定文案表**，逐字照抄（`金属为骨，光影为魂` → `Metal as the framework, light as the soul.`）。
6. **两种做法都不许**：❌ 每张图重复列一遍角标；❌ 全文一处都不留。
...
| **尺寸板块多个尺寸挤在一行** | 图上引线对不上数值、读者分不清谁是谁。该块写 `"kind": "size"`，脚本按同一分隔符自动拆成一行一个尺寸（要关闭写 `"no_split": true`）；`verify_docx.py` 的 `size rows` 应为 **0** |
| 尺寸板块没有中文提示 | `kind` 写了 `"size"` 就会自动带固定那句（客户指定口径）；只有显式写 `"tip_cn": ""` 才没有 —— 别省，运营靠它判断「部位文案不作上架用」 |
| **部位文案拼成 `Base — Metal`** | 原图标签**本身就是文案**，用破折号/斜杠把部位与材质拼起来读着生硬（客户原话：「比较生硬」）。要连贯翻译成英文名词短语：`碳素钢底座` → `Metal Base`、`白玉磨砂玻璃` → `Frosted White Jade Glass`；语序「材质 + 部位」，拼写照第〇节。闸门 `glossary_check.py` 会对拼接式 FAIL |
| 提示段被并进图片段 | 提示是块内**第二个** `<p>`，但首个 1pt 占位段仍必须在最前面（见排版原则 a） |

## 文件

| 文件 | 用途 |
|---|---|
| `scripts/build_bilingual_doc.py` | 主生成脚本，读作业 JSON 一次跑完全文档（`pairs` → 中英两栏对照表；尺寸块 `kind="size"` 自动带中文提示、自动把一行多个尺寸拆成一行一个尺寸） |
| `scripts/job.example.json` | 作业文件示例（BD111 实战，含 brand / target_lang / 角标去重 / 尺寸块 `kind`+`tip_cn` 与句对写法） |
| `scripts/slice_long_images.py` | 长图切分：`--src` 单张长图走**自动模式**，`--plan` 走指定模式；含空白带检测、校验条带、纯空段丢弃 |
| `scripts/verify_docx.py` | 不依赖渲染服务的排版/内容校验（图片/表格/行数/英文空格/行线/行距/内边距/**尺寸行数/提示段数**） |
| `scripts/color_caliber.py` | 颜色口径闸门：同名多译色（砂岩黑 / 中国红）按**产品材质**判定；判不出来就返回待确认问题、由生成器中止（退出码 2）。可单独运行诊断 |
| `scripts/glossary_check.py` | **固定文案口径闸门**：扫作业 JSON，查出违禁变体（Carbon Steel / Lamp Post / Backplate / 单用 Shade…）与固定文案漏译、逐字不一致（含标点）；退出码 2 = 必须先改文案 |
| `scripts/edsdk_docx.py` | editor_sdk 调用封装（容错解析、批量段落操作、`style_tables` 表格布局、安全覆盖） |
| `references/translation-playbook.md` | **第〇节固定文案表（客户指定，最高优先级）**、意译原则、材质口径、颜色标准译名表、术语表、单位换算、断句与交付自检清单 |
